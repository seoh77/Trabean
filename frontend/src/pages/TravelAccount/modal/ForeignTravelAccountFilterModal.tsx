...
const ForeignTravelAccountFilterModal: React.FC<
  ForeignTravelAccountFilterModalProps
> = ({ accountId, onClose }) => {
  ...
  const handleFilterOption = () => {
    ...
    onClose();
    nav(
      `/accounts/travel/foreign/${accountId}/detail?startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
    );
  };

  // Account 서버 외화 여행통장 생성일 조회 API fetch 요청
  useEffect(() => {
    const fetchTravelAccountData = async () => {
      try {
        const response = await client().get(
          `/api/accounts/travel/foreign/${accountId}/created`,
        );
...
export default ForeignTravelAccountFilterModal;
