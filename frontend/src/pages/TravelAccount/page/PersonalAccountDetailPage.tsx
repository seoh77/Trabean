...
import PersonalAccountFilterModal from "../modal/PersonalAccountFilterModal";

const PersonalAccountDetailPage: React.FC = () => {
  ...
  const [personalAccountDetailData, setPersonalAccountDetailData] =
    useState<DomesticTravelAccountDetailData>(); // 개인 통장 상세조회 상태관리
  ...
  const handleTransferBalance = () => {
    navigate(`/accounts/personal/${accountId}/detail/transfer`);
  };

  // Account 서버 개인 통장 상세 조회 API fetch 요청
  useEffect(() => {
    const getPersonalAccountDetailData = async () => {
      try {
        const response = await client().get(
          `/api/accounts/personal/${accountId}`,
          { params: { startDate, endDate } },
        );
        setPersonalAccountDetailData(response.data);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
  ...
  const renderGroupedTransactions = () => {
    if (!personalAccountDetailData?.transactionList) return null;

    const groupedTransactions = groupByDate(
      personalAccountDetailData.transactionList,
    );
      ...
      <div className="px-4 py-2">
        <div className="text-center">
          <div className="font-bold text-xl p-2">
            {personalAccountDetailData?.accountName}
          </div>
          <div className="font-bold p-2">
            ₩{personalAccountDetailData?.accountBalance.toLocaleString()}
          </div>
        </div>
        <div className="py-4">
          <button
